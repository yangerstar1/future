"""Real R06B keyframe/cost check; reuse the verified R06 Cycles configuration."""
...
for f,width,samples,neutral,blur in [(1,1280,32,False,False),(192,1280,32,False,False),(97,1920,24,False,True),(1,960,24,True,False)]:
 S.frame_set(f);S.camera=bpy.data.objects['R06B_DEPARTURE_TRACK'];bpy.context.view_layer.update();config(width,samples,blur)
 S.view_layers[0].material_override=bpy.data.materials['Neutral inspection clay'] if neutral else None
 p=OUT/('R06B_f%03d%s.png'%(f,'_NEUTRAL' if neutral else ''));S.render.filepath=str(p)
 ...
 rows.append(row);(OUT/'OBSERVATIONS.json').write_text(json.dumps(rows,indent=2));print('R06B_FRAME_SAVED',p.name,seconds,flush=True)
