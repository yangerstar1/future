"""Skyfold Dock R01: native editable geometry. Provisional, never auto-qualifies art.
...
R=1600.0; WIDTH=1800.0; SEED=270910; random.seed(SEED)
OUT=Path(os.environ.get('SKYFOLD_OUT','output/r01')).resolve(); OUT.mkdir(parents=True,exist_ok=True)
...
for i in range(48):
 ...
 for y in [18,450,900,1350,1782]:
  a=F@Vector((0,y,0));b=F@Vector((0,min(y+420,1800),0));beam('External axial backbone',a,b,20,28,DARK,'01_RING_STRUCTURE')
...
for i in range(144):
 ...
 for j in range(15):
  ...
  if min(abs(y-a) for a in [300,700,1100,1500])<51:continue
  wrap=min(t,2*math.pi-t)
  if wrap<.60 and y<1780:continue
...
# R01: a continuous 1.44m-gauge service route; elevated near platform joins the dock.
...
# R01 removes seven optional foreground frame/conduit pieces that occluded the hero.
...
# Preserve metre-scale maintenance geometry while changing the still-unfrozen G2 vantage.
...
# Broad opening skylight approximation exposes the cavity rather than hiding it in fog.
...
hero_pos=Vector((-694,-323,168.75));hero_target=Vector((200,1650,1200))
...
camera('O04_CRAFT',(-695,-302,169.35),(-688,-286,168.5),41)
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'skyfold-r01.blend'),compress=True)
manifest['scene_sha256']=hashlib.sha256((OUT/'skyfold-r01.blend').read_bytes()).hexdigest();(OUT/'BUILD-MANIFEST.json').write_text(json.dumps(manifest,indent=2))
