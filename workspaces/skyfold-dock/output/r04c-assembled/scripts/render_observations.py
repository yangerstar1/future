...
scene_hash=hashlib.sha256((OUT/'skyfold-r00.blend').read_bytes()).hexdigest()
assert scene_hash==man['scene_sha256']
views=[('O01_HERO',960,16,False),('O01_SECOND',640,12,False),('O01_HERO',960,16,True),('O03_REAR',640,12,False),('O04_CRAFT',800,24,False),('O04_INTERFACE',640,16,False),('O05_ROUTE',640,12,False)]+[('O06_%03d'%(i*25),576,10,False) for i in range(5)]
...
for name,width,samples,clay in views:
 ...
 rec={'observation':label,'candidate':'R00_PROVISIONAL','source_sha':os.environ['GITHUB_SHA'],'scene_sha256':scene_hash,'image_sha256':hashlib.sha256(p.read_bytes()).hexdigest(),'camera':name,'camera_matrix':[list(row) for row in S.camera.matrix_world],'dimensions':dims,'samples':samples,'seconds':elapsed,'render_engine':'CYCLES_CPU','compositing':False,'visual_verdict':'NOT_OBSERVED','review_derivative':derivative(p)}
