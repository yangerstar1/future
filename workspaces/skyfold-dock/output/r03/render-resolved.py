...
man=json.loads((OUT/'BUILD-MANIFEST.json').read_text());digest=hashlib.sha256((OUT/'skyfold-r03.blend').read_bytes()).hexdigest()
assert man['scene_sha256']==digest
views=[('O01_HERO',1200,24,False),('O01_CAMERA_B',960,20,False),('O01_CAMERA_C',960,20,False),('O01_SECOND',960,20,False),('R01_O01_HERO',960,16,False),('O01_HERO',1200,24,True),('O03_REAR',960,20,False),('O04_CRAFT',1200,32,False),('O04_INTERFACE',960,24,False),('O05_ROUTE',1200,24,False),('O05_LIFT',960,20,False),('O05_LIFT_R03_DIAGNOSTIC',960,20,False)]+[('O06_%03d'%(i*25),800,16,False) for i in range(5)]
records=[]
for name,w,samples,clay in views:
 ...
 row={'observation':label,'candidate':'R03_NEAR_PIER_REPAIR','source_sha':os.environ['GITHUB_SHA'],'scene_sha256':digest,'image_sha256':hashlib.sha256(path.read_bytes()).hexdigest(),'camera':name,'matrix':[list(r) for r in SC.camera.matrix_world],'lens':SC.camera.data.lens,'dimensions':dimensions,'samples':samples,'seconds':elapsed,'engine':'CYCLES_CPU','compositing':False,'visual_verdict':'NOT_OBSERVED'}
...
print('R03_OBSERVATIONS_SAVED',len(records),'NOT_A_VISUAL_PASS')
