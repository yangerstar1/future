...
if MODE=='build':
 ...
 habitat=bpy.data.objects['R04_HABITAT_ROTATION'];habitat.animation_data_clear();habitat.rotation_euler=(0,0,0)
 bpy.context.view_layer.update()
 # Preserve all existing non-ship geometry. Only loading bridges are stowed.
 environment=[o for o in S.objects if o.type in {'MESH','CURVE','FONT'} and o not in C['04_FREIGHTER'].objects]
 ...
 # The legacy berth and matched ports are preserved in the parent snapshot.
 # This shot starts already clear of all piers: no rigidly docked ship drifts away.
 stowed=[]
 ...
 # Reuse the existing manufactured shell; assign ship-only materials.
 recipes={
 ...
 # Plated nose follows the actual existing hull facets, not a new unrelated ship.
 # Separate plate solids add edge response without changing the envelope substantially.
 platecount=0
 for f in list(hull.data.polygons)[2:26]:
  ...
  radial=Vector((mid.x-center.x,0,mid.z-center.z))
  if normal.dot(radial)<0:normal=-normal
  a=[mid+(p-mid)*.91+normal*1.1 for p in pts];vs=a+[v-normal*.8 for v in a]
  me=bpy.data.meshes.new('R06 formed bow plate');me.from_pydata(vs,[],[(0,1,2,3),(7,6,5,4),(0,4,5,1),(1,5,6,2),(2,6,7,3),(3,7,4,0)]);me.update()
  me.materials.append(mats['Pearl ceramic-coated alloy']);o=bpy.data.objects.new('R06 formed bow plate %02d'%platecount,me);C['04_FREIGHTER'].objects.link(o)
 ...
 # Active nozzle cores are placed at the already inspected aft engine positions.
 ...
 # Lighting revision: directional sunlight, modest blue fill; no many-light flat wash.
 oldlights=[]
 ...
 cams=[follow('R06_FRONT_QUARTER',(-1050,-1750,320),38),follow('R06_CROSS_RING',(-1280,-420,180),35),follow('R06_WIDE_CONTEXT',(-1400,-1250,450),30)]
 ...
 for f in range(1,193):
  ...
  # Gentle camera orbit while maintaining subject size; no roll or zoom trick.
  for o in cams:
   original={'R06_FRONT_QUARTER':Vector((-1050,-1750,320)),'R06_CROSS_RING':Vector((-1280,-420,180)),'R06_WIDE_CONTEXT':Vector((-1400,-1250,450))}[o.name]
   o.location=original+Vector((100*t,-100*t,35*t));o.keyframe_insert(data_path='location',frame=f)
 S.camera=cams[1];config(1280,32)
 # Explicit motion/clearance observations from actual evaluated matrices.
 checks=[];staticname='Continuous inhabited ring shell';static0=None
 for f in [1,25,49,73,97,121,145,169,192]:
  ...
  ringmat=matrix(bpy.data.objects[staticname]);static0=static0 or ringmat;assert ringmat==static0
 ...
 for n,d in oldcams.items():assert n in bpy.data.objects
 ...
 if MODE=='probe':
  views=[('R06_FRONT_QUARTER',1,1280,32,False),('R06_CROSS_RING',1,1280,32,False),('R06_WIDE_CONTEXT',1,1280,32,False),('R06_CROSS_RING',97,1280,32,False),('R06_CROSS_RING',192,1280,32,False),('R06_CROSS_RING',1,960,24,True)]
