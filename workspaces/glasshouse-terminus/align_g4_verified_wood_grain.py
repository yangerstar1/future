"""COAST-R05: fix the measured U/V wood-grain mismatch, not a new world.
The actual source scan and source UV inspection were retrieved and reviewed.
Only three table rims and seven sideboard members receive dedicated coordinates
and copied shaders. All original UVs, source scan bytes and geometry are retained.
"""
 ...
 for o in sorted(s.objects,key=lambda x:x.name):
  ...
  if o.type=='MESH':
   ...
   for name in original_uvs[o.name]:
    a=array('f',[0])*(len(o.data.loops)*2);o.data.uv_layers[name].data.foreach_get('uv',a);h.update(name.encode());h.update(a.tobytes())
...
if MODE=='build':
 ...
 original_uvs={o.name:[uv.name for uv in o.data.uv_layers] for o in s.objects if o.type=='MESH'}
 before=protected(original_uvs)
 report={'stage':'G4','revision':'COAST-R05','source_sha':os.environ['GITHUB_SHA'],'run_id':os.environ['GITHUB_RUN_ID'],
  ...
  'basis':'The packed finished veneer JPG was actually opened and hash verified: dominant grain is horizontal U. Native read-only inspection confirmed member length and table circumference instead mapped to V. Fix only that coordinate error, with a same-source native before/after observation.',
  'source_scan_sha256':'c0ba49a1fbc7b6ef93dd87abd83eee21eab611a6b291a0bdc6e4cf67b468158f',
  'changes':[],'g4_stage_pass':False,'human_acceptance':False}
 materials={}
 for srcname,newname in [(EDGE,'G4COAST_R05_circumferential_walnut'),(JOINERY,'G4COAST_R05_member_aligned_walnut')]:
  ...
  raw=bytes(n['diff'].image.packed_file.data);assert hashlib.sha256(raw).hexdigest()==report['source_scan_sha256']
  ...
  outgoing=[q.to_socket for q in list(old.outputs['UV'].links)]
  assert len(outgoing)==4,('Current shader links differ from read-only evidence',srcname,len(outgoing))
  for socket in outgoing:l.new(uv.outputs['UV'],socket)
  normals=[q for q in n if q.type=='NORMAL_MAP'];assert len(normals)==1 and normals[0].space=='TANGENT'
  normals[0].uv_map=UVNAME
  fine=n['Vector Math.001'];assert fine.type=='VECT_MATH' and fine.operation=='MULTIPLY'
  assert tuple(fine.inputs[1].default_value)==(450.0,28.0,1.0)
  fine.inputs[1].default_value=(28,450,1)
  # The old procedural pores followed V; align them to the actual scan's U grain.
  # No roughness, coat, color, light, displacement or original scan edit.
  materials[srcname]=m
 for previous in inspection['objects']:
  ...
  # Verify the inspected points before modification, rather than trusting object names.
  for f in previous['faces']:
   face=o.data.polygons[f['polygon']]
   actual=np.asarray([o.data.uv_layers.active.data[li].uv[:] for li in face.loop_indices])
  ...
  o.data=o.data.copy();old_uv=o.data.uv_layers.active
  coords=np.empty(len(o.data.loops)*2,dtype=np.float32);old_uv.data.foreach_get('uv',coords);coords=coords.reshape(-1,2)
  original=coords.copy();affected=[]
  for face in o.data.polygons:
   if o.data.materials[face.material_index].name==srcname:affected.extend(face.loop_indices)
  assert len(affected)>0
  coords[affected]=coords[affected][:,[1,0]]
  new=o.data.uv_layers.new(name=UVNAME);new.data.foreach_set('uv',coords.reshape(-1));o.data.uv_layers.active=old_uv
  # Retain old active UV and original top-face materials. Only copied target shaders use the new layer.
  for i,m in enumerate(o.data.materials):
   if m.name==srcname:o.data.materials[i]=materials[srcname]
  for face in o.data.polygons:
   if o.data.materials[face.material_index]!=materials[srcname]:continue
   if o.name not in CABINET:continue
   co=np.asarray([o.data.vertices[o.data.loops[li].vertex_index].co[:] for li in face.loop_indices])
   uvdata=np.asarray([new.data[li].uv[:] for li in face.loop_indices])
   axes=[i for i in range(3) if i!=int(np.argmax(np.abs(np.asarray(face.normal))))]
   design_axis=1 if o.name.startswith('G3_cabinet_rail') else 2
   if design_axis not in axes:continue
   A=np.column_stack([co[:,axes],np.ones(len(co))]);fit=np.linalg.lstsq(A,uvdata,rcond=None)[0];direction=fit[axes.index(design_axis)]
   assert abs(direction[0])>100*max(abs(direction[1]),1e-9),'Member length still maps to cross-grain'
  row={'object':o.name,'new_uv':UVNAME,'old_uv_preserved':old_uv.name,'changed_corner_count':len(affected),'old_material':srcname,'new_material':materials[srcname].name,'coordinate_edit':'Only target face corners swap U/V. Three rims now map circumference to U; sideboard member length maps to U.'}
  report['changes'].append(row)
 after=protected(original_uvs);assert before==after,'Protected source geometry, old UVs, other material assignments or observation conditions changed'
 report.update(protected_before=before,protected_after=after,protected_scope='All source mesh positions/topology/face material indices/smoothing, old UV layers, source transforms/parents/ray flags, other object material bindings, camera/lamp/exposure/world identity/shutter. No claim of exhaustive Blender-property comparison.')
 ...
 shutil.copy2(BASE/'build-report.json',OUT/'COAST-R04-build-report.json')
 s['g4_revision']=REV;s['g4_verified_wood_parent']=PARENT;s.camera=bpy.data.objects['G3_D02']
 ...
 for cam,name,res,samples in jobs:
  ...
  row.update(native_render=True,master_sha256=expected,revision='COAST-R04_BASELINE' if MODE=='before' else 'COAST-R05',max_bounces=10,transmission_bounces=8)
