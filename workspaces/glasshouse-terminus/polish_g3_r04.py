"""G3 R04 art challenger. One existing structural bay, not G4.
Use the exact R03 source; protect the full world, saved cameras and movement.
Light diagnosis 34121257143 identifies the interior studio card, not the glass.
"""
...
A=bpy.data.collections['A_complete_glasshouse']
old_objects=list(s.objects)
...
def geometry_signature():
    ...
    return h.hexdigest()
protected=geometry_signature()
report={'stage':'G3','revision':'R04','status':'ART_CHALLENGER_NOT_ACCEPTED',
 ...
 'scope':'Complete existing x=-4..0 structural bay and its existing door interface, not the full hall.',
 'diagnosis_run':34121257143,'diagnosis_zip_sha256':'e4d28113c0f9204326a2e14aeed927262b8c12bac8e07c61f2ac47b08282d88a',
 'source_arches':[],'material_edits':[],'light_edits':[],
 'g4_allowed':False,'human_acceptance':False,'browser_gate':'BLOCKED_UNCHANGED'}
arches=[];ribs=[]
for ob in A.objects:
    ...
    if -4.01<=x<=.01 and ob.name.startswith(('Primary_elliptical_arch','Secondary_glazing_rib')):
        ...
        assert lo[1]<-6.4 and hi[1]>6.4 and hi[2]>10,'Different arch geometry; inspect first'
...
seal=bpy.data.materials['G3_EPDM_seal'];glass=bpy.data.materials['G3_clear_6mm_glass']
wetglass=bpy.data.materials['G3_exterior_rivulet_glass']
...
# Test X bounds with tolerance; do not reject full arches because midpoint Y is -epsilon.
# Whole-span members receive the new finish only within this structural bay.
def finish(ob,mat):
    old=ob.active_material;lo,hi=box(ob)
    if lo[0]>=-4.20 and hi[0]<=.20:new=mat
    else:
        new=old.copy();new.name='G3R04_zone_'+ob.name
        n=new.node_tree.nodes;l=new.node_tree.links;output=n.get('Material Output')
...
families=('Primary_elliptical_arch','Secondary_glazing_rib','Load_column','Column_capital',
 'Column_base_plinth','Wall_glazing_mullion','Portal_transom_mullion','Wall_transom','Wall_stone_sill',
 'Longitudinal_roof_purlin','Continuous_eaves_gutter','Curved_roof_glazing','Side_wall_glass')
for ob in list(A.objects):
    ...
    if ob.name.startswith(('Curved_roof_glazing','Side_wall_glass')):
        mat=wetglass if ob.name.startswith('Side_wall_glass') else glass
    elif ob.name.startswith(('Column_base_plinth','Wall_stone_sill')):mat=stone
    else:mat=enamel
    finish(ob,mat)

# Real pressure-cap sections on the existing three secondary arch axes.
# Structural I-beams stay intact; caps, gaskets and fasteners form separate layers.
for ob in ribs:
    lo,hi=box(ob);x=(lo[0]+hi[0])/2
    path=[]
    for j in range(73):
        a=j*math.pi/72;n=Vector((0,math.cos(a)/6.5,math.sin(a)/5.1)).normalized()
        c=Vector((x,6.5*math.cos(a),5+5.1*math.sin(a)))
        path.append(tuple(c+n*.045))
    sweep('G3R04_glazing_pressure_cap',path,[(-.032,-.005),(.032,-.005),(.032,.008),(-.032,.008)],enamel)
    for j in range(4,70,12):
        a=j*math.pi/72;n=Vector((0,math.cos(a)/6.5,math.sin(a)/5.1)).normalized()
        c=Vector((x,6.5*math.cos(a),5+5.1*math.sin(a)))
        cylinder('G3R04_pressure_cap_fixing',c+n*.052,c+n*.059,.008,bronze,12)
# Eaves-to-arch splice plates, with real washers and fasteners on both sides.
for ob in arches:
    lo,hi=box(ob);x=(lo[0]+hi[0])/2
    for side in [-1,1]:
        y=side*6.5
        cube('G3R04_arch_splice_plate',(x+.108,y,5.065),(.016,.215,.42),enamel,.006)
...
# Remove the unmotivated studio card from the lighting design, not from ray visibility.
# Single-card ablation already identified its reflection. The glass remains unchanged.
for name,power in [('G3_interior_softbox',0),('G3_pendant_light',260),('G3_cafe_practical',14),('G3_door_practical',90)]:
    ...
    report['light_edits'].append({'object':name,'before':lamp.data.energy,'after':power})
    lamp.data.energy=power
opal=material('G3R04_opal_sconce',(.76,.70,.59),rough=.37,emission=2.0)
new_lights=[]
for x in [-4,0]:
    ...
    light=area('G3R04_sconce_light',(x,6.025,3.72),(-2,2.6,2.1),24,.28,(1,.84,.66))
    new_lights.append(light.name)
night=json.loads(s['g3_night_lights'])+new_lights
s['g3_night_lights']=json.dumps(night)
report['lighting_design']='Studio fill retired after isolated attribution. Existing real pendant/table/train fixtures strengthened; two attached opal column sconces added. No light-path exclusions, no exposure change.'

# A supplementary camera past the hall leaves; old failed camera remains saved.
# First-hit visibility is measured as well as frustum inclusion; never move the doors.
...
lo1,hi1=box(threshold);lo2,hi2=box(housing)
...
for lens in range(34,19,-1):
    door.data.lens=lens
    q=[world_to_camera_view(s,door,p) for p in pts]
    if all(v.z>0 and .04<=v.x<=.96 and .04<=v.y<=.96 for v in q):break
else:raise RuntimeError('Interface frame does not fit; no scene changes to fit camera')
...
assert geometry_signature()==protected,'Original geometry, transforms or cameras changed'
report['protected_geometry_before']=protected;report['protected_geometry_after']=geometry_signature()
# Restore actual source timeline markers before saving the candidate.
with bpy.data.libraries.load(str(SOURCE),link=False) as (src,dst):
    pass
# Marker data is reconstructed from the source scene's known saved film boundaries.
# Read source marker snapshot captured before clearing, rather than invent markers.
...
s['g3_revision']='R04 motivated illumination and completed arch bay'
s['g3_r04_parent_sha256']=EXPECTED
