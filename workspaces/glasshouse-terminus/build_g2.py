"""Complete G2 whitebox, never final art. Dimensions in metres, Z up.
A-E coexist. 32 x 13 m hall, .04 rail head, 1.15 platform/car floor,
16 m original single carriage. Source animation 30 FPS, 28 seconds.
"""
...
def rail_tangent(distance):return (rail_pos(distance+.02)-rail_pos(distance-.02)).normalized()

def rail_offset(distance,offset,z):
    p=rail_pos(distance);t=rail_tangent(distance);n=Vector((-t.y,t.x,0));p+=n*offset;p.z=z
    return tuple(p)

with component('E_cliff_foundation'):
    rng=random.Random(23);n=40;verts=[];radial=[1+rng.uniform(-.06,.06) for _ in range(n)]
    ...
    for x in range(-17,18,2):
        for y in [-8.8,15.0]:cylinder('Terrace_guard_post',(x,y,.0),(x,y,1.12),.043,mat['secondary'])
    for y in [-8.8,15.0]:
        for z in [.4,.72,1.10]:curve('Terrace_guard_rail',[(-17,y,z),(17,y,z)],.035,mat['secondary'])
    for i in range(7):
        h=FLOOR*(7-i)/8
        cube('Hall_entry_step',(16.35+i*.31,0,(h-.025)/2),(.34,3.1,h+.025),mat['stone'],.008)
...
with component('E_curved_viaduct_and_track'):
    for a in range(-70,-19,10):
        ...
        for at in [a,b]:
            ...
            foot=cube('Viaduct_bedrock_footing',(pt.x,pt.y,-22.6),(2.9,5.2,1.1),mat['stone'],.08);foot.rotation_euler.z=post.rotation_euler.z
    ...
    for side in [-1,1]:sweep('Running_rail',[rail_offset(-72+i*.25,side*RAIL_C,0) for i in range(361)],rail_profile,mat['dark'],frame='Z')
    for i in range(151):
        d=-72+i*.6;p=rail_pos(d);t=rail_tangent(d)
        ob=cube('Track_sleeper',(p.x,p.y,-.22),(.22,2.35,.20),mat['interior'],.015);ob.rotation_euler.z=math.atan2(t.y,t.x)
        if d<-19 and i%3==0:
            for side in [-1,1]:
...
with component('A_complete_glasshouse'):
    ...
    for x in range(-16,17):
        ...
        sweep('Primary_elliptical_arch' if major else 'Secondary_glazing_rib',path,primary if major else secondary,mat['structure'] if major else mat['secondary'])
        for y in [-6.5,6.5]:
            if major:
            ...
            elif y>0 and -3.28<x<-.72:
                cube('Portal_transom_mullion',(x,y,4.67),(.061,.076,.59),mat['secondary'],.003)
            else:cube('Wall_glazing_mullion',(x,y,3.05),(.061,.076,3.62),mat['secondary'],.003)
    for j in range(13):
        a=j*pi/12;cube('Longitudinal_roof_purlin',(0,6.5*cos(a),5.0+5.1*sin(a)),(32.18,.075,.085),mat['secondary'],.005)
    for x in range(-16,16):
        for j in range(12):
            verts=[]
            for xx in [x+.045,x+.955]:
                for k in range(5):
                    a=(j+(k/4)*.95+.025)*pi/12;verts.append((xx,6.507*cos(a),5.0+5.107*sin(a)))
            ob=mesh('Curved_roof_glazing',verts,[(k,k+1,k+6,k+5) for k in range(4)],mat['glass'])
            ob.modifiers.new('Glass thickness 6mm','SOLIDIFY').thickness=.006
    # Split boundaries explicitly at the portal: no mullion, glass or transom across the opening.
    for side in [-1,1]:
        y=side*6.5;edges=sorted(set(list(range(-16,17))+([-3.28,-.72] if side==1 else [])))
        for a,b in zip(edges,edges[1:]):
            center=(a+b)/2;portal=side==1 and -3.28<center<-.72
            if not portal:cube('Wall_stone_sill',(center,y,FLOOR+.18),(b-a-.008,.20,.36),mat['stone'],.008)
            for lo,hi in ([(4.32,4.94)] if portal else [(1.54,3.37),(3.39,4.94)]):
                thin_panel('Side_wall_glass',[(a+.025,y,lo),(b-.025,y,lo),(b-.025,y,hi),(a+.025,y,hi)],mat['glass'])
            for z in ([4.92] if portal else [1.53,3.38,4.92]):cube('Wall_transom',(center,y,z),(b-a+.01,.08,.065),mat['secondary'],.003)
        cube('Continuous_eaves_gutter',(0,y,5.02),(32.55,.25,.18),mat['structure'],.018)
        for x in [-15.8,15.8]:curve('Downpipe',[(x,y,4.99),(x,y+side*.21,4.75),(x,y+side*.21,FLOOR+.1)],.055,mat['secondary'])
    # Curved top boundary fills each end all the way to the roof, not stair-stepped rectangles.
    for x in [-16.03,16.03]:
        for j in range(10):
            y0=-6.5+j*1.3;y1=y0+1.3;yc=(y0+y1)/2
            lo=4.0 if x>0 and abs(yc)<1.31 else FLOOR+.38
            verts=[(x,y0+.025,lo),(x,y1-.025,lo)]
            for k in range(9):
                y=y1-.025-(y1-y0-.05)*k/8;z=5.0+5.10*math.sqrt(max(0,1-(y/6.5)**2))-.022
                verts.append((x,y,z))
            thin_panel('End_wall_glazing',verts,mat['glass'])
            top=5.0+5.1*math.sqrt(max(0,1-(y0/6.5)**2))
    ...
    for side,hinge in [('left',-3.2),('right',-.8)]:
        ...
        sign=1 if side=='left' else -1;before=set(bpy.data.objects)
...
with component('B_cafe_and_waiting_proxies'):
    for x,y in [(-3.15,3.3),(-10.2,3.3),(7.4,3.2)]:
        ...
        for yy in [y-1.02,y+1.02]:
            ...
            cube('Cafe_chair_back_proxy',(x,yy+(.27 if yy>y else -.27),FLOOR+.79),(.61,.12,.62),mat['interior'],.06)
            for dx in [-.23,.23]:
...
with component('D_complete_enterable_train'):
    ...
    for side in [-1,1]:
        for k in range(len(edges)-1):
            ...
            if not is_door:
                ...
                ob=mesh('Car_curved_lower_shell',verts,[(j,j+1,j+5,j+4) for j in range(3)],mat['train']);ob.modifiers.new('Sheet thickness','SOLIDIFY').thickness=.06
        ...
        trim=[(-7.25,-4.67),(-3.33,7.25)] if side==-1 else [(-7.25,7.25)]
        for a,b in trim:cube('Car_waist_trim',((a+b)/2,side*1.40,2.02),(b-a,.039,.034),mat['secondary'],.009)
    verts=[]
    for x in [-7.25,7.25]:
        for j in range(49):
            a=-pi/2+j*pi/48;verts.append((x,1.39*sin(a),3.35+.72*cos(a)))
    ob=mesh('Car_complete_barrel_roof',verts,[(j,j+1,j+50,j+49) for j in range(48)],mat['train']);ob.modifiers.new('Roof shell thickness','SOLIDIFY').thickness=.065
    for sign in [-1,1]:
        for side in [-1,1]:
            ...
            thin_panel('Cab_upper_cheek',[(sign*7.25,side*1.39,3.27),(sign*8.01,side*1.10,3.18),(sign*8.01,side*1.10,3.56),(sign*7.25,side*1.39,3.36)],mat['train'],.06)
        points=[]
        for j in range(25):
            y=-1.10+2.2*j/24;points.append((sign*(8.07-.15*(y/1.1)**2),y))
        for z0,z1,role in [(1.10,2.10,'train'),(2.13,3.19,'glass'),(3.23,3.56,'train')]:
            vv=[(x,y,z) for z in [z0,z1] for x,y in points]
            ob=mesh('Cab_front_glass' if role=='glass' else 'Cab_front_fairing',vv,[(j,j+1,j+26,j+25) for j in range(24)],mat[role]);ob.modifiers.new('Cab skin thickness','SOLIDIFY').thickness=.012 if role=='glass' else .07
        vv=[]
        for end in [0,1]:
            for j in range(25):
                a=-pi/2+j*pi/24;y=(1.39 if end==0 else 1.10)*sin(a)
                xx=sign*7.25 if end==0 else sign*(8.07-.15*(y/1.1)**2)
                z=3.35+.72*cos(a) if end==0 else 3.56
                vv.append((xx,y,z))
        ob=mesh('Cab_complete_roof_loft',vv,[(j,j+1,j+26,j+25) for j in range(24)],mat['train']);ob.modifiers.new('Cab roof thickness','SOLIDIFY').thickness=.07
    ...
    for side in [-1,1]:
        spans=[(-7.1,-4.67),(-3.33,7.1)] if side==-1 else [(-7.1,7.1)]
        for a,b in spans:cube('Car_inner_wainscot',((a+b)/2,side*1.295,1.70),(b-a,.053,.97),mat['interior'],.015)
    ...
    for side,xc in [('left',-4.31),('right',-3.69)]:
        root=bpy.data.objects.new('Train_door_'+side,None);bpy.context.collection.objects.link(root);root.location=(xc,-1.43,FLOOR);before=set(bpy.data.objects)
    ...
    for local_x in [-5.0,5.0]:
        ...
        for axle_x in [-.91,.91]:
            axle=bpy.data.objects.new('Rolling_wheelset',None);bpy.context.collection.objects.link(axle);axle.parent=bogie;axle.location=(axle_x,0,.42);before=set(bpy.data.objects)
...
for frame in range(1,841):
    ...
    rear=rail_pos(distance-5);front=rail_pos(distance+5);mid=(rear+front)/2;yaw=math.atan2((front-rear).y,(front-rear).x)
    ...
    for bogie,offset in bogies:
        ...
        bogie.location=(c*p.x+sn*p.y,-sn*p.x+c*p.y,0);bogie.rotation_euler.z=math.atan2(tg.y,tg.x)-yaw
    ...
    for door,xc,side in doors:
        door.location=(xc+side*.65*opening,-1.43-.095*opening,FLOOR);door.keyframe_insert('location',frame=frame)
    if frame in [1,91,211,301,315,348,391,541,661,840]:motion.append({'frame':frame,'seconds':t,'track_distance':distance,'body_location':list(mid),'body_yaw':yaw,'door_open_fraction':opening})
s.frame_set(451)

with component('QA_scale_and_route'):
    for x,y in [(-1.1,3.0),(-2,8.55),(9.4,9.3)]:
        for dx in [-.09,.09]:cylinder('Human_scale_leg',(x+dx,y,FLOOR),(x+dx,y,FLOOR+.78),.07,mat['proxy'],12)
    ...
    curve('QA_continuous_passenger_route',route,.034,mat['route'])['diagnostic_only']=True
...
cams={
 'C01':camera('C01_exterior_hero',(32,52,23),(-10,4,1),40),
 ...
 'C09':camera('C09_car_aisle_failure_check',(-3,TRACK_Y,2.7),(7.3,TRACK_Y,2.2),30)
}
film={
...
walk_keys=[(13,(-2,1.1,2.76),(-2,8,2.73)),(15.5,(-2,5.55,2.76),(-2,11,2.75)),(18,(-2,10.92,2.76),(-2,13,2.75)),(19.3,(-2,12.1,2.76),(3.2,12.1,2.73)),(20.6,(-.2,12.1,2.76),(-2,9,2.9)),(22,(1,12.1,2.76),(-2,3.4,3.25))]
def mix(a,b,t):return tuple((1-t)*aa+t*bb for aa,bb in zip(a,b))
previous_quaternions={}
for frame in range(1,841):
    t=(frame-1)/30
    if t<3:
        cam=film['HERO'];pos=mix((34,54,24),(31,50,22.5),t/3);target=(-10,4,1)
    elif t<7:
        cam=film['DETAIL'];pos=mix((-4.7,8.6,3.05),(-4.35,8.4,2.98),(t-3)/4);target=(-2.8,5.2,2.5)
    elif t<13:
        cam=film['ARRIVAL'];pos=mix((-11,9.4,2.7),(-8.8,9.4,2.7),(t-7)/6);target=(-2,TRACK_Y,2.4)
    elif t<22:
        cam=film['WALK']
        ...
        cam=film['REVEAL'];pos=mix((30,44,19),(35,53,30),(t-22)/6);target=(-12,2,-.4)
    cam.location=pos;cam.rotation_mode='QUATERNION';q=(Vector(target)-cam.location).to_track_quat('-Z','Y')
    if cam.name in previous_quaternions and q.dot(previous_quaternions[cam.name])<0:q.negate()
    cam.rotation_quaternion=q;previous_quaternions[cam.name]=q.copy()
    cam.keyframe_insert('location',frame=frame);cam.keyframe_insert('rotation_quaternion',frame=frame)
for frame,key in [(1,'HERO'),(91,'DETAIL'),(211,'ARRIVAL'),(391,'WALK'),(661,'REVEAL')]:s.timeline_markers.new('SHOT_'+key,frame=frame).camera=film[key]
s.camera=film['HERO'];s.frame_set(1)
layout={'hall_length':32.0,'hall_width':13.0,'rail_head_z':.04,'platform_z':FLOOR,'car_floor_z':FLOOR,'platform_edge_y':10.65,'train_near_side_y':TRACK_Y-1.4,'door_world_x':-2.0,'car_length':16.37,'walking_shot_seconds':[13,22]}
s['task']='glasshouse-terminus-premium-scene';s['phase']='G2_WHITEBOX_NOT_FINAL_ART';s['layout_json']=json.dumps(layout)
...
(OUT/'layout-and-route.json').write_text(json.dumps({'layout':layout,'walk_keys':walk_keys,'clearance_design_metres':{'platform_to_train':TRACK_Y-1.4-10.65,'train_door_clear_width':1.20,'station_portal_clear_width':2.40,'aisle_width':.64},'note':'Design parameters do not constitute art acceptance. Verify actual views and continuous route.'},indent=2))
metrics=[];s.timeline_markers.clear();s.frame_set(451)
for ob in bpy.data.collections['QA_scale_and_route'].objects:ob.hide_render=False
for key in ['C01','C02','C03','C04','C05','C06','C07','C09']:metrics.append(render(s,cams[key],OUT/(key+'.png'),res=(1280,800),samples=24,frame=451))
# True section in a disposable in-memory diagnostic; the saved master is untouched.
for ob in bpy.data.collections['QA_scale_and_route'].objects:ob.hide_render=True
...
for ob in list(s.objects):
    ...
    plane=ob.matrix_world.inverted()@Vector((-2,0,0));normal=(ob.matrix_world.to_3x3().transposed()@Vector((1,0,0))).normalized()
